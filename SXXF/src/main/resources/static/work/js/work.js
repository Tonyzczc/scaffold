...
layui.use(['element', 'form', 'table', 'layer', 'laydate','tree', 'util'], function () {
    ...
    table.on('tool(test)', function (obj) {
        ...
        //编辑
            ...
            searchByStationName();
            form.render();
            var publicPic = data.publicPic;
            if(publicPic != null){
                $("#upload").css("display", "none");
                $("#houseImg").css("display", "block");
                $("#publicPic").attr("src",publicPic);
                var myURL = new URL(publicPic);
...
layui.use('upload', function(){
    ...
    var uploadInst = upload.render({
        elem: '#upload' //绑定元素
        ,url: 'uploadHallsPic' //上传接口
        ,done: function(res){
            var server = res.data.server;
            var fileName = res.data.fileName;
            $("#upload").css("display", "none");
            $("#houseImg").css("display", "block");
            $("#publicPic").attr("src",server+"/"+fileName);
            $("#img").text(fileName);
            $("#img").val(fileName);
            //上传完毕回调
        }
...
function authorityFormSave() {
    ...
    $.post(ctx + "work/save", authorityForm, function (data) {
